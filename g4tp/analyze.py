...
def longitudinal_profile(path, axis="z", nbins=140, step_size="256 MB", verbose=True):
    ...
    for n_batch, cols in io.iterate_flat(path, [zkey, "step_edep"], step_size=step_size):
        seen += n_batch
        z = cols[zkey] / io.MM_PER_CM        # mm -> cm
        w = cols["step_edep"]
        depth = sign * (z - z0)
        keep = depth >= 0
...
def summarize(path, outdir="g4tp_analysis", make_plots=True, depth_axis="z"):
    ...
    primE = np.asarray(sc.get("primaryE", np.zeros(n)), float)
    edep = np.asarray(sc.get("totalEdep", np.zeros(n)), float)
    nsteps = np.asarray(sc.get("nSteps", np.zeros(n)), float)
    pdg0 = int(sc["primaryPDG"][0]) if "primaryPDG" in sc and len(sc["primaryPDG"]) else 0
    lines += [
